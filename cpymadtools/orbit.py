...
from cpymadtools.constants import LHC_CROSSING_SCHEMES
...
def lhc_orbit_variables() -> Tuple[List[str], Dict[str, str]]:
    ...
    """
    logger.debug("Returning (HL)LHC orbit variables")
    on_variables = (
        "crab1",
...
def setup_lhc_orbit(madx: Madx, scheme: str = "flat", **kwargs) -> Dict[str, float]:
    ...
    for orbit_variable in variables:
        ...
        logger.debug(f"Setting orbit variable '{orbit_variable}' to {variable_value}")
    ...
    for special_variable, copy_from in special.items():
        ...
        logger.debug(f"Setting special orbit variable '{special_variable}' to {special_variable_value}")
...
def correct_lhc_orbit(
    ...
    **kwargs,
    ...
    for default_kicker in ("kicker", "hkicker", "vkicker", "virtualcorrector"):
        logger.debug(f"Disabling default corrector class '{default_kicker}'")
    ...
    for _ in range(iterations):
        logger.debug("Doing orbit correction for Y then X plane")
